...
int main(int argc, char **argv) {
...

	{
		...
		while(fgets(chunk, sizeof(chunk), input) != NULL) {
			...
			ptr = strtok(chunk, " ");
			while(ptr != NULL) {
				data[triple_count] = strtoull(ptr, NULL, 10);
				triple_count++;
				ptr = strtok(NULL,  " ");
			}
			...
			for (j = 4096 * (part - 1); j < (uint64_t)4096 * (part); ++j) {
			    ...
			    if(triple_count == 8) {
			    	...
			    	entries = get_stables_double_triple_loot(world_seed, &tmp_pos1, &tmp_pos2, loot, data[3], data[6]);
			    } else if(triple_count == 11) {
					tmp_pos1 = { triple_one_x, triple_one_z };
			    	tmp_pos2 = { triple_two_x, triple_two_z };
			    	tmp_pos3 = { triple_three_x, triple_three_z };
			    	entries = get_stables_loot(world_seed, &tmp_pos1, &tmp_pos2, &tmp_pos3, loot,data[3],data[6],data[9]);
				...
				//printf("%" PRIi64 " %d ; early seed out\n", world_seed,iron_ingots + iron_nuggets / 9);
