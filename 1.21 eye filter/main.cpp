...
void test() {
    int chunk_x = -95;
    int chunk_z = -86;
    uint64_t world_seed = 2108865602411596520LL;
...
void get_eyes(int chunk_x, int chunk_z, uint64_t world_seed, Pos *frame_positions, int *eyes) {
    ...
    //     printf("%ld ; /teleport @a %d ~ %d ; %d\n", world_seed, chunk_x<<4, chunk_z<<4, eyes);
    // }
}

void eye_test(uint64_t seed, int x, int z) {
    XRSR128 rng;
    uint64_t feature_seed;

    for (int r_x = -7; r_x < 7; ++r_x) {
        for (int r_z= -7; r_z < 7; ++r_z) {
            feature_seed = set_feature_seed(seed, x + r_x, z + r_z, 19, 4);
            xrsr_seed(&rng, feature_seed);
            skip(&rng);
...
int main() {
    ...
    int startChunkX, startChunkZ, tmp;
    char splitee[] = ",";
    int pearls, rods;
    uint64_t seed;
    int required_eyes;
    ...
    int eyes[12];
    float stronghold_distance;
    int superflat_strongholds[6] = { -19, -108, -95, 73, 127, 49 };
    ...
    while(fgets(chunk, sizeof(chunk), input) != NULL) {
        ...
        seed = strtoull(strtok(chunk, splitee), NULL, 10);
        rods = strtoull(strtok(NULL, splitee), NULL, 10);
        pearls = strtoull(strtok(NULL, splitee), NULL, 10);
        ...
        initFirstStronghold(&sh, MC_1_21_1, seed);
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            // stronghold_distance = std::sqrt(sh.pos.x * sh.pos.x + sh.pos.z*sh.pos.z);
            // if (stronghold_distance >= 2000.0f) {
            //     continue;
            // }

            // startChunkX = superflat_strongholds[k*2];
            // startChunkZ = superflat_strongholds[k*2+1];

            // stronghold_generator::StrongholdGenerator::getFirstPosFast(seed, startChunkX, startChunkZ);
            strongholdGenerator.generate(seed, startChunkX, startChunkZ);
            ...
            int portal_start_x = portalRoomPiece->getWorldX(3,  8);
            int portal_end_x = portalRoomPiece->getWorldX(7,  12);
            ...
            int portal_start_z = portalRoomPiece->getWorldZ(3,  8); 
            int portal_end_z = portalRoomPiece->getWorldZ(7,  12);
            ...
            // printf("%ld ; /teleport @a %d ~ %d\n", seed, portal_start_x, portal_start_z);
            // continue;
            for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
                for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
                    ...
                    get_eyes(chunkX, chunkZ, seed, frame_positions, eyes);
                }
            }

            int eyes_count = 0;
            for (int indeks = 0; indeks < 12; ++indeks) {
                eyes_count += eyes[indeks];
            }

            if (eyes_count >= required_eyes) {
                printf("%" PRIi64 " ; %d ; /teleport @a %d ~ %d\n", seed, eyes_count, portal_start_x, portal_start_z);
            }
        }
        // break;
    }   
