...
int main(int argc, char **argv)
{
...

	{
		...
		struct StructureSeed
		{
			...
			uint64_t data3, data6, data9;
			int tripleCount;
		};

		std::vector<StructureSeed> structSeeds{};
		std::cout << "Reading structure seeds" << std::endl;
		{
		...
		std::cout << "Read structure seeds" << std::endl;
		
		const int numThreads = omp_get_max_threads();
		const int numSeedsPerThread = (structSeeds.size() - 1) / numThreads + 1;
		...
		std::ofstream ofile(std::string("output_") + std::to_string(part));
		while (finishedThreads < numThreads) {
			...
			for (int i = 0; i < numThreads; ++i) {
				...
				std::cout << "Thread #" << i << ": " << threadProgress[i].seedIndex << std::endl;
			}
		}
		ofile.close();
	}
